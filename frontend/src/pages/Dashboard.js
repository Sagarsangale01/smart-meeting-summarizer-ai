...
import {
  Box, Grid, Typography, Divider, Chip, Fade,
    ...
    >
      <Box
        sx={{
          ...
          background: `rgba(${glow}, 0.12)`, border: `1px solid rgba(${glow}, 0.25)`,
...
function Dashboard() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: 'var(--bg-deep)', position: 'relative', fontFamily: 'var(--font-primary)' }}>
      ...
      {/* Global loading bar */}
      {isLoading && (
        <LinearProgress sx={{ height: 2, background: 'rgba(99,102,241,0.1)', '& .MuiLinearProgress-bar': { background: 'linear-gradient(90deg, #6366f1, #8b5cf6, #10b981)' } }} />
      )}
      ...
      <Box sx={{ maxWidth: 1280, mx: 'auto', px: { xs: 2, sm: 3, md: 5 }, py: { xs: 3, md: 5 }, position: 'relative', zIndex: 1 }}>
        ...
        <Box sx={{ mb: 5, animation: 'fadeInUp 0.6s ease both' }}>
          ...
          >
            Smart Meeting{' '}
            <Box component="span" sx={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Summarizer
            </Box>
          </Typography>
          <Typography sx={{ fontSize: { xs: '0.875rem', md: '1rem' }, color: '#64748b', maxWidth: 520, lineHeight: 1.7 }}>
            Paste any meeting transcript and get an instant AI-structured summary —
            topics, decisions, and action items with owners.
          </Typography>
          <Box sx={{ display: 'flex', gap: 1, mt: 2, flexWrap: 'wrap' }}>
            {[
              ...
              <Chip key={f.label} label={`${f.icon}  ${f.label}`} size="small"
              ...
              <>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                  <Box>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 0.3 }}>
                      <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>
                        AI-extracted · Click any item to edit inline
                      </Typography>
                      {summary.ragUsed && (
                        <Chip
                          ...
                          size="small"
                          sx={{ height: 20, fontSize: '0.65rem', fontWeight: 700, color: '#8b5cf6', background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)' }}
                        />
                      )}
                    </Box>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
                    {/* Stats */}
                    <StatCard icon={<BarChartIcon sx={{ fontSize: 15, color: '#6366f1' }} />} label="Total Items" value={totalItems} color="#6366f1" glow="99,102,241" />
                    {processingTime && (
                      <StatCard icon={<TimerOutlinedIcon sx={{ fontSize: 15, color: '#10b981' }} />} label="Processed in" value={`${processingTime}s`} color="#10b981" glow="16,185,129" />
                    )}
                    ...
                    <Tooltip title="View meeting history">
                      <Button
                        ...
                        sx={{
                          ...
                          borderRadius: '9px', px: 1.8, py: 0.7,
                ...
                >
                  <Box sx={{ width: 8, height: 8, borderRadius: '50%', background: '#10b981', boxShadow: '0 0 8px rgba(16,185,129,0.7)', flexShrink: 0, animation: 'pulse-glow 2s ease-in-out infinite' }} />
                  <Typography sx={{ fontSize: '0.75rem', color: '#64748b', lineHeight: 1.5 }}>
                    <Box component="span" sx={{ color: '#94a3b8', fontWeight: 600 }}>Hallucination guard active.</Box>{' '}
      ...
      >
        <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.5px', color: '#64748b' }}>
          ENTRATA TECHNICAL TASK
        </Typography>
        <Typography sx={{ fontSize: '0.85rem', mt: 0.5, color: '#94a3b8' }}>
          Submitted by <Box component="span" sx={{ color: '#6366f1', fontWeight: 700 }}>Sagar Sangale</Box>
        </Typography>
      </Box>
